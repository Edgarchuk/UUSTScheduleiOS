...
enum API {
    ...
    static let url = URL(string: "hhtp://isu.ugatu.su/api/schedule/")!
    fileprivate static let login = "arpakit"
    fileprivate static let password = "vPDPhQAE"
    
    static func getGroups() async throws -> [Group] {
        async let response = AF.request(url, method: .get,  headers: .groups).serializingData()
        ...
        var groups = values.map({Group(json: $0!)})
